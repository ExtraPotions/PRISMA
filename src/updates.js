EXP.Updates = (() => {
  ...
  async function check(force = false) {
    ...
    try {
      ...
      return last = { state: 'checked', available: compare(latest, EXP.VERSION), latest, details };
    } catch (error) { EXP.Core.safeError(Object.assign(error, { code: 'UPDATE_CHECK' }), 'prisma'); return last = { state: 'failed', available: false, latest: null, details: [] }; }
  }
  return Object.freeze({ check, status: () => ({ ...last, details:last.details.slice() }) });
})();
