...
test('PRISMA uses the shared version-scoped updater and concise release parsing', () => {
  ...
  assert.match(updates, /productId:\s*'prisma'/);
  assert.match(updates, /repository:\s*'ExtraPotions\/PRISMA'/);
  assert.match(updates, /endpoint:\s*'https:\/\/api\.github\.com\/repos\/ExtraPotions\/PRISMA\/releases\/latest'/);
